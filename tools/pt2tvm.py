...
# 已经jit trace后的模型导入
model=torch.jit.load("../model/model_jitt.pt")
# print(model)
...
lib.export_library("../model/policy_x64_cpu.so")#将生成的共享库导出为.so文件，这个文件可以在x64架构的 CPU 上运行
...
lib.export_library("../model/policy_arm64_cpu.so", cc='/usr/bin/aarch64-linux-gnu-g++-9')#指定了编译器cc参数为aarch64-linux-gnu-g++-9，这是一个适用于ARM64的C++编译器，通常用于交叉编译

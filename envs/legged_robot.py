...
class LeggedRobot(BaseTask):
    ...
    def _compute_torques(self, actions):
        ...
        # ----------------------------------------------------------------------
        # SIMPLIFIED: No pose interpolation, always use default_dof_pos
        # This matches the sim2sim deployment logic for consistency
        # ----------------------------------------------------------------------
        bias = self.default_dof_pos  # Always use the same default pose
        # ----------------------------------------------------------------------

        if self.cfg.domain_rand.randomize_lag_timesteps:
            self.lag_buffer = torch.cat([self.lag_buffer[:,1:,:].clone(),actions_scaled.unsqueeze(1).clone()],dim=1)
            joint_pos_target = self.lag_buffer[self.num_envs_indexes,self.randomized_lag,:] + bias
        else:
            joint_pos_target = actions_scaled + bias

        #joint_pos_target =  self.default_dof_pos#no net output
        self.action_avg=joint_pos_target#doghome
        
        # Enforce URDF limits on target positions to prevent self-collision or limit violation
        joint_pos_target = torch.clamp(joint_pos_target, self.dof_pos_limits[:, 0], self.dof_pos_limits[:, 1])
    ...
    def reset_idx(self, env_ids):
        ...
        # Add zero velocity command ratio monitoring
        zero_vel_mask = torch.norm(self.commands[:, :3], dim=1) < self.cfg.rewards.command_dead
        self.extras["episode"]['zero_vel_ratio'] = zero_vel_mask.float().mean()
        self.extras["episode"]['mean_cmd_lin_vel_x'] = self.commands[:, 0].abs().mean()
        self.extras["episode"]['mean_cmd_lin_vel_y'] = self.commands[:, 1].abs().mean()
        self.extras["episode"]['mean_cmd_ang_vel'] = self.commands[:, 2].abs().mean()
    ...
    def compute_ref_state(self):#生产参考轨迹
        ...
        # Scale reference amplitude by velocity to prevent stepping in place
        velocity_magnitude = torch.norm(self.commands[:, :2], dim=1)
        # Map 0.0-0.5 m/s to 0.0-1.0 scale factor
        amplitude_scale = torch.clamp(velocity_magnitude / 0.5, 0.0, 1.0)
        
        scale_1 = self.cfg.rewards.target_joint_pos_scale * amplitude_scale
        scale_2 = 2 * scale_1
        
        # Interpolate between Standing (st) and Walking default poses for smooth transition
        scale_u = amplitude_scale.unsqueeze(1)
        self.ref_dof_pos[:,0:12] = self.default_dof_pos_st * (1.0 - scale_u) + self.default_dof_pos * scale_u
    ...
    def _resample_commands(self, env_ids):#zhiling
        """ Randommly select commands of some environments - REWRITTEN FSR FIX """
        num_envs = len(env_ids)
        
        # 1. Sample base velocities
        self.commands[env_ids, 0] = torch_rand_float(self.command_ranges["lin_vel_x"][0], self.command_ranges["lin_vel_x"][1], (num_envs, 1), device=self.device).squeeze(1)
        self.commands[env_ids, 1] = torch_rand_float(self.command_ranges["lin_vel_y"][0], self.command_ranges["lin_vel_y"][1], (num_envs, 1), device=self.device).squeeze(1)
        self.commands[env_ids, 2] = torch_rand_float(self.command_ranges["ang_vel_yaw"][0], self.command_ranges["ang_vel_yaw"][1], (num_envs, 1), device=self.device).squeeze(1)
        ...
        # Zero out commands for stopped envs
        stop_indices = env_ids[should_stop]
        if len(stop_indices) > 0:
            self.commands[stop_indices, 0] = 0.
            self.commands[stop_indices, 1] = 0.
            self.commands[stop_indices, 2] = 0.
            self.commands[stop_indices, 3] = 0.
            
        # 5. Deadzone logic for non-stopped envs
        keep_indices = env_ids[~should_stop]
        if len(keep_indices) > 0:
            dead_zone = 0.05 # Hardcoded deadzone
            small_vel = torch.norm(self.commands[keep_indices, :2], dim=1) < dead_zone
            small_indices = keep_indices[small_vel]
            if len(small_indices) > 0:
                self.commands[small_indices, :2] = 0.
